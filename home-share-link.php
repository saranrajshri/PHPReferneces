...
$share_url=strip_tags(@$_POST['share_url']);
...
{
	mysql_query("INSERT INTO shared_links VALUES('','$log_user_name','$user','$typ_user_name','$username','$share_url','$d','no') ");
}
}
?>
<div class="header2">
...
<a href="home-share-link.php?profile_id=<?php echo $user;?>"class="active-3"><img src="./img/share-link.png" width="20" height="20">Share link</a>
...
<input type="text" name="share_url" placeholder="Enter the link you want to share" class="filled">
<input type="submit" name="share-link" value="Share" class="button blue">
</form>
</div>
<hr style="width:56.5%;margin-left:280px;margin-top:10px;">
<?php
$getinfo=mysql_query("SELECT * FROM shared_links WHERE username_id='$user' ORDER BY id DESC");
while($get=mysql_fetch_array($getinfo)){
	$username_from=$get['username'];
	$post_link_id=$get['id'];
	$username_from_id=$get['username_id'];
	$user_name_to_id=$get['user_name_to_id'];
	$user_name_to=$get['user_name_to'];
	$link=$get['link_url'];
$get_user_info = mysql_query ("SELECT * FROM users WHERE id='$username_from_id'");
$get_info = mysql_fetch_assoc($get_user_info);
$profilepic_info = $get_info['profile_pic'];
echo'<div id="posting"><img src ="./img/share-link.png" width="20" height="20"><a href="profile.php?profile_id='.$username_from_id.'">'.$username_from.'</a> <i style="font-size:13px;font-weight:bold">shared a link</i><a href="http://'.$link.'" style="margin-left:10px;padding-top:10px;font-size:12px;">'.$link.'</a></div>
<hr style="width:56%;margin-left:280px;"><br>';
}				
?>
<!--right-->
<div class="rightside">
