...
<?php
$user_from="";
$friendRequests= mysql_query("SELECT * FROM messages WHERE user_to ='$user' ORDER BY id DESC");
$numrows=mysql_num_rows($friendRequests);
if ($numrows==0) {
echo"<center><p style='padding-top:10px;'>You have no messages at this time</p></center>";
...
while ($get_row = mysql_fetch_assoc($friendRequests)) {
	$id=$get_row['id'];
	$accepted_from=$get_row['user_from_name'];
	$from_id=$get_row['user_from'];
	$date=$get_row['date'];
...
echo'<img src="./userdata/profile_pics/'.$from_pic.'" style="border:1px solid #ccc;margin:10px" width="50" height="50"><h1 style="color:#045dad;word-spacing:5px;margin-top:-70px;margin-left:60px">'.$accepted_from.' : '.$message.'</h1>
